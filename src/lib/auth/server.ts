 ...
 *   - Sandbox live preview: explicit server-side OAuth credentials, deriving
 *     the preview's `https://*.grok-sandbox.com`
 ...
 *
 * NEVER import this from client code — it pulls in `pg`, server credentials and
...
import { resolveOAuthCredentials } from "./oauth-config";
...
// Both preview and deployment must receive their own server-side credentials.
// Missing configuration with auth enabled is an error, never a shared identity.
const grokIssuer = env("GROK_AUTH_ISSUER") ?? GROK_ISSUER_DEFAULT;
const oauthCredentials = resolveOAuthCredentials(process.env);
const grokClientId = oauthCredentials?.clientId;
const grokClientSecret = oauthCredentials?.clientSecret;

/** True when federated sign-in is active (real auth is enforced). */
export const authConfigured = oauthCredentials !== null;
